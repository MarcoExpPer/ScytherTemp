...
void UTargetingComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if( character )
	{
		...
		posibleTargets = GetPosibleTargets( character->targetRange );
...
TArray<AActor*> UTargetingComponent::FindActorsInRange( float range )
{
	...
	if( range > 0 )
	{
		...
		targetNet = (ATargetingNet*) GetWorld()->SpawnActor( targetNetClass, &GetOwner()->GetActorTransform() );
...
bool UTargetingComponent::IsVisible( AActor* target )
{
	...
	//Como ignoramos al propio jugador y al enemigo, si algo colisiona con el rayo, es que hay algun obstaculo entre medias.
	if( GetWorld()->LineTraceSingleByChannel( hit, GetOwner()->GetActorLocation(), target->GetActorLocation(), CollisionChannel, CollisionParams ))
	{
		return false;
	}
	return true;
	...
	//TODO:
	// No todo tiene que colisionar con el rayo. Los coliders sin meshes no bloquean la vision y para el jugador
	// puede parecer que pierde el focus gratuitamente.
}
...
void UTargetingComponent::ClassifyPosibleTargets()
{
	for( int i = 0; i < posibleTargets.Num(); i++ )
	{
		...
		if( horizontalDir <= character->playerVisionAngle.X/2 && verticalDir <= character->playerVisionAngle.Y/2 )
		{
...
AActor* UTargetingComponent::ChooseTarget()
{
	...
	else if( !character->onlyTargetVisionCone )
	{
...
{
	AActor* closestEnemy = nullptr;
	float closestDistance = character->targetRange * 2;

	for( int i = 0; i < list.Num(); i++ )
	{
		float dist = FVector::Distance( list[i]->GetActorLocation(), GetOwner()->GetActorLocation() );
...
void UTargetingComponent::SwitchTarget()
{
	...
	const FVector loc = playerTarget->GetActorLocation() + (FVector( switchTargetInput.Y, -switchTargetInput.X, 0 ) * 1000 );
	const FTransform transform = FTransform( FRotator(0, 0, 0), loc, FVector(1, 1, 1));
	targetNet = (ATargetingNet*) GetWorld()->SpawnActor( targetNetClass, &transform );

	if( targetNet )
	{
		//Asignamos el rango definido a la esfera
		targetNet->SetCollisionRadius( character->targetRange );
		//Cogemos todos los actores que se encuentran en la esfera
		targetNet->GetOverlappingActors( overlappingActors );
		//Destruimos el collider
		targetNet->Destroy();
