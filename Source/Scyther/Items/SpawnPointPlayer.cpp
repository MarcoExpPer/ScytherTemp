		...
		if( comp->ComponentHasTag( "PlayerBody" ) )
		{
			...
			if( IsValid( playerController ) )
			{
				...
				if( CurrentSpawnPoint.GetLocation() != CurrentCollider.GetLocation() && CurrentSpawnPoint.GetRotation() != CurrentCollider.GetRotation() && CurrentSpawnPoint.GetScale3D() != CurrentCollider.GetScale3D() )
				{
					...
					Manager->manaToRespawnThePlayerWith = playerController->scytherPlayer->skillComponent->currentLightMana;

					FCheckpointActivatedEvent.Broadcast();
					playerController->scytherPlayer->healthComponent->changeHealth( HealthIncreaseWhenGetToCheckpoint, DamageModes::HEALING, true );
				}
			}
		}
	}
