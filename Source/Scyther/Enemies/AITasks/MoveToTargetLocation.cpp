...
EBTNodeResult::Type UMoveToTargetLocation::ExecuteTask( UBehaviorTreeComponent& ownerComp, uint8* NodeMemory )
{
	...
	if( ctrl != nullptr )
	{
		...
		EPathFollowingRequestResult::Type move = ctrl->moveToPosition( bb->GetValueAsVector( targetLocation.SelectedKeyName ) );
		if(move == 1 && targetLocation.SelectedKeyName == "PositionHorizontalAttack" )
		{
			bb->SetValueAsBool( "IsReadyToAttack", true );
		}
		/*if((ValidShake == true  ) )
		{
