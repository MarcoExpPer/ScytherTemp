...
bool ULocationCloserThanADistance::CalculateRawConditionValue( UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory ) const
{
	...
	if( ctrl != nullptr )
	{
		...
		float traceSize = ( location - pawnLocation ).Size();
		float minimumDistanceValue = bb->GetValueAsFloat( minimumDistance.SelectedKeyName );
		return traceSize <= minimumDistanceValue;
	}
	return false;
}
